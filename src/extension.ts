...
const PATTERN = 'db/migrate/data/*.rb'
...
function onFilesFound(files: Uri[]) {
  ...
  if (paths.length > 0) {
    ...
    window.showInformationMessage('No data migrations found!')
  }
}

export function activate(context: ExtensionContext) {
    const latestMigration = commands.registerCommand('extension.railsLatestDataMigration', () => {
